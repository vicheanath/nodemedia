...
const NodeCoreUtils = __importStar(require("@nodemedia/core/service/Utils"));
const FlvPacket = {
    create: (payload = 0, type = 0, time = 0) => {
        return {
            header: {
...
class NodeFlvSession {
    constructor(config, req, res) {
        ...
        this.numPlayCache = 0;
        Context_1.default.sessions.set(this.id, this);
    }
    run() {
        ...
        Logs_1.default.info(`[${this.TAG} connect] id=${this.id} ip=${this.ip} args=${JSON.stringify(urlInfo.query)}`);
        Context_1.default.nodeEvent.emit("preConnect", this.id, this.connectCmdObj);
        if (!this.isStarting) {
            this.stop();
            return;
        }
        Context_1.default.nodeEvent.emit("postConnect", this.id, this.connectCmdObj);
        if (method === "GET") {
    ...
    stop() {
        if (this.isStarting) {
            this.isStarting = false;
            let publisherId = Context_1.default.publishers.get(this.playStreamPath);
            if (publisherId != null) {
                Context_1.default.sessions.get(publisherId).players.delete(this.id);
                Context_1.default.nodeEvent.emit("donePlay", this.id, this.playStreamPath, this.playArgs);
            }
            Logs_1.default.info(`[${this.TAG} play] Close stream. id=${this.id} streamPath=${this.playStreamPath}`);
            Logs_1.default.info(`[${this.TAG} disconnect] id=${this.id}`);
            Context_1.default.nodeEvent.emit("doneConnect", this.id, this.connectCmdObj);
            this.res.end();
            Context_1.default.idlePlayers.delete(this.id);
            Context_1.default.sessions.delete(this.id);
        }
    }
    onReqClose() {
        this.stop();
    ...
    reject() {
        Logs_1.default.info(`[${this.TAG} reject] id=${this.id}`);
        this.stop();
    }
    onPlay() {
        Context_1.default.nodeEvent.emit("prePlay", this.id, this.playStreamPath, this.playArgs);
        if (!this.isStarting) {
            return;
        }
        if (this.config.auth !== undefined && this.config.auth.play) {
            ...
            if (!results) {
                Logs_1.default.info(`[${this.TAG} play] Unauthorized. id=${this.id} streamPath=${this.playStreamPath} sign=${this.playArgs.sign}`);
                this.res.statusCode = 403;
                this.res.end();
                return;
            }
        }
        if (!Context_1.default.publishers.has(this.playStreamPath)) {
            Logs_1.default.info(`[${this.TAG} play] Stream not found. id=${this.id} streamPath=${this.playStreamPath} `);
            Context_1.default.idlePlayers.add(this.id);
            this.isIdling = true;
            return;
        }
        this.onStartPlay();
    ...
    onStartPlay() {
        let publisherId = Context_1.default.publishers.get(this.playStreamPath);
        let publisher = Context_1.default.sessions.get(publisherId);
        ...
        this.isIdling = false;
        this.isPlaying = true;
        Logs_1.default.info(`[${this.TAG} play] Join stream. id=${this.id} streamPath=${this.playStreamPath} `);
        Context_1.default.nodeEvent.emit("postPlay", this.id, this.playStreamPath, this.playArgs);
    }
...
exports.default = NodeFlvSession;
