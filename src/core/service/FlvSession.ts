...
//
import URL from "url";
import Logger from "@nodemedia/core/utils/Logs";
import context from "@nodemedia/core/service/Context";
import * as NodeCoreUtils from "@nodemedia/core/service/Utils";

const FlvPacket = {
  create: (payload: any = 0, type = 0, time = 0) => {
    return {
      header: {
...
class NodeFlvSession {
  ...
  private id: any;
  private ip: any;
  private playStreamPath: any;
  private playArgs: any;
  private isStarting: boolean;
  private isPlaying: boolean;
  private isIdling: boolean;
  private connectTime: Date;
  private TAG: string;
  private numPlayCache: number;
  private connectCmdObj: any;
  ...
  run() {
    ...
    let urlInfo = URL.parse(this.req.url, true) as any;
  ...
  stop() {
    if (this.isStarting) {
      ...
      Logger.info(`[${this.TAG} disconnect] id=${this.id}`);
  ...
  reject() {
    Logger.info(`[${this.TAG} reject] id=${this.id}`);
    this.stop();
  }
